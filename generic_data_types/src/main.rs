...
// Мономорфизация — это процесс превращения обобщённого кода в конкретный код
// путём подстановки конкретных типов, использующихся при компиляции.
// В этом процессе компилятор просматривает все места, где вызывается обобщённый
// код, и генерирует код для конкретных типов, использовавшихся для вызова в
// обобщённом.

// Мономорфизированная версия кода выглядит примерно так:
